...
repos = [l.strip() for l in open(f'{D}/repos_wave2.txt') if l.strip()]
done = set()
if os.path.exists(f'{D}/harvest_w2_done.txt'):
    done = set(l.strip() for l in open(f'{D}/harvest_w2_done.txt'))
out = open(f'{D}/threads_w2.jsonl', 'a')
donef = open(f'{D}/harvest_w2_done.txt', 'a')
