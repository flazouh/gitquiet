...
- A comment written on a line is held unsent until the whole review is submitted, and the label on it says `PENDING`, which reads as waiting on somebody else. The control that sends it is on a different tab from the Conversation the Participant is usually on when they reply. [Discussion 10369](https://github.com/orgs/community/discussions/10369), 463 upvotes and 164 comments over four years, is people reporting comments that sat invisible for days or months, change requests that were missed and merged over, and reviewers who thought they were being ignored. The same Reply button posts immediately or holds, depending on which tab it is on and whether an unsubmitted review already exists, so the rule cannot be learnt by using it.
...
51. As a Reviewer, I want a comment written outside a Review Pass to reach the Author at once, so that being read does not depend on my remembering a second step.
52. As an Author, I want my reply to a thread on my own pull request to post at once, so that answering a reviewer never begins a review I then have to submit.
53. As a Participant, I want the interface never to call an Unsent Comment pending, so that I cannot read my own unfinished work as somebody else's turn.
54. As a Reviewer, I want my Unsent Comments counted in Your Move alongside the control that sends them, so that finishing a Review Pass costs one interaction from where I already am.
55. As a Participant, I want Unsent Comments from an earlier session shown the moment I open the pull request, so that a Review Pass I abandoned last week cannot stay invisible.
56. As a Participant, I want to be stopped before I resolve a thread whose reply is still unsent, so that resolving cannot read as dismissing somebody in silence.
...
Rendering is React and Tailwind. Colour and type follow Appearance × Pack (see
`docs/spec/themes.md`): Gitquiet is the default, Inter is the typeface, and the
reader can switch packs in Settings. Icons remain Octicons. Tailwind's preflight
is not imported, and its utilities are imported unlayered — a layered rule loses
to GitHub's unlayered element rules regardless of specificity.
...
- The conversation route returns Alive channel tokens only, and no timeline at all. Two page-data routes carry the timeline between them: `page_data/timeline` lists every item by id, type and author with no text, and `page_data/issue_comments` returns the bodies as a bare array — `authorLogin`, `body`, `bodyHtml`, `createdAt`, `isHidden`. No internal GraphQL and no Relay global ID are needed, contrary to the guess this spike replaced. Verified against `flowline-labs/flowline#1934`, which had one remark and no review threads and so read as silent until this route was added.
...
### Comments and Unsent Comments

Two paths, and which one a comment takes is decided by what the Participant was doing rather than by which tab they were on.

A comment written outside a Review Pass posts at once. That covers replying to a thread, answering a Bot Finding, and anything an Author says on their own pull request. There is no state to leave behind and nothing to submit afterwards, because the thing the Participant did was say something to somebody.

A comment written inside a Review Pass is held, and held deliberately: batching is the one part of GitHub's model the discussion defends, because a reviewer's first reaction to a file is often wrong by the end of the diff and the Author should not be sent to fix it. Story 23 stands. What the discussion is about is not the batch; it is that the batch is invisible and its exit is somewhere else.

So the batch is never silent. Three rules govern it.

The word is never GitHub's. `PENDING` is banned in copy, and so is Draft, which GitHub already spends on a pull request's own state. An Unsent Comment is marked with what is actually true of it — that only the Participant can see it — and the Slack pattern the discussion keeps pointing at is the model.

Unsent Comments are an Attention Item in Your Move, carrying their count, and the control that sends them sits on the row. This is the design constraint applied to the case that motivated it: zero interactions to know the comments are unsent, one interaction to send them. Because the Court is computed from the snapshot rather than from the current session, a Review Pass abandoned last week is on the Control Center the next time the pull request opens, which is the failure mode the discussion reports most.

Leaving is guarded. Navigating away or closing with Unsent Comments warns first, and resolving a thread whose own reply is unsent is refused, because the two together are what makes a Participant look like they dismissed somebody without a word.

Reading them costs nothing. The changes payload already carries `viewerPendingReview` as `{id, comments}` — it is in `fixtures/github/changes.json` — so the count and the bodies are in a payload the extension fetches anyway, with no second request and no separate route.

Sending is one field, and it is now known which way it points. `create_review_comment` takes `submitBatch`: `true` posts at once, `false` holds the comment in the Participant's own unsubmitted review. The gateway sends `true`, so what it writes today is already visible to everybody, and the Review Pass path will be the same route with the field flipped. Both directions were exercised against a scratch pull request and are recorded in `docs/spec/github-write-api.md`, along with the reason nothing here may lean on the response: a comment that was held and a comment that was sent come back as the same 200 with the same thread body, and only `viewerPendingReview` on the next read tells them apart.
