...
The regression test failed before the patch: the tree published an empty selection and performed three updates. It passes after the patch, with at most two updates: selection and scrolling. All 4,775 tests pass. A fresh frozen install produces identical patched runtime bytes. Code-quality review found no issue. Browser timing verification passed for selection CPU: all three pairs improved, with a median of 766.1 to 672.6 ms. Median diff readiness increased from 99.8 to 104.0 ms. Keep the patch for lower CPU work; zero dropped frames remains unproven. See `scripts/performance/RESULTS.md` for the retry conditions and full results.
