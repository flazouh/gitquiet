...
const ChecksSummary = ({ standing }: { readonly standing: RunStanding }) => {
  const art = useArt()
  const CheckFailed = art["check-failed"]
  const CheckPassed = art["check-passed"]
  ...
  switch (standing.kind) {
    ...
    case "running": {
      ...
      const Art = checkArt(art, standing.started ? "running" : "queued")
      return (
    ...
    case "passed":
      return (
        <span className="flex items-center gap-1.5">
          <CheckPassed size={12} className={CHECK_TONE.succeeded} />
...
const CheckRow = ({ check, onOpen }: { readonly check: Check; readonly onOpen: () => void }) => {
  const Art = checkArt(useArt(), check.state)
...
export const Checks = ({
  ...
  /** Reads the steps a check ran as, which is what the dialog opens as. */
