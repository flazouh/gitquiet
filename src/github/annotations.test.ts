...
describe("finding the check run behind a check", () => {
  test("takes it from the job GitHub links to", () => {
    expect(
      checkRunIn(check("/octo-org/octo-repo/actions/runs/30297001180/job/90080390889?pr=1392"))
    ).toBe("90080390889")
  })

  test("has nothing to say about a check that is not an Actions job", () => {
    expect(checkRunIn(check("https://review.example.com/review/octo-repo/pull/1392"))).toBeUndefined()
  })
})
...
describe("reading what GitHub wrote against a check", () => {
  ...
  /*
   * The bug this was written for: a run page draws a notice with `octicon-info` in
   * `color-fg-muted`, which was in no row of the table, so all ten of the worked run's
   * notices were reported as failures and drawn in the failure colour. Their Checks tab
   * uses the accent for the same thing, so both are read.
   */
  test("reads a notice as a notice, in either colour they draw it", () => {
    expect(notesIn(real.replace("color-fg-danger", "color-fg-accent"))[0]?.level).toBe("notice")
    expect(notesIn(real.replace("color-fg-danger", "color-fg-muted"))[0]?.level).toBe("notice")
  })
...
describe("the place in the log a note points at", () => {
  ...
  test("is nothing when the link is not one of those", () => {
    expect(Option.isNone(spotIn("/octo-org/octo-repo/actions/runs/1/job/2"))).toBe(true)
