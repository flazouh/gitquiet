...
import { involvedFrom, involvedIn, standingsIn, withStandings } from "./involved"
...
const row = (over: Partial<WorkingSetRow> = {}): WorkingSetRow => ({
  ...
  repoNameWithOwner: "OpenRouterIncubator/ori",
  permalink: "https://github.com/OpenRouterIncubator/ori/pull/1457",
...
const one = (over: Partial<WorkingSetRow> = {}) =>
  Option.getOrThrow(involvedFrom("needs-action", row(over)))
...
describe("reading a Working Set row as an Involved Pull Request", () => {
  ...
  test("a row whose repository name will not split is dropped, not fatal", () => {
    ...
    expect(involvedFrom("needs-action", row({ repoNameWithOwner: "noslash" }))).toEqual(
      Option.none()
    )
    expect(involvedFrom("needs-action", row({ repoNameWithOwner: "a/b/c" }))).toEqual(Option.none())
    expect(involvedFrom("needs-action", row({ repoNameWithOwner: "/ori" }))).toEqual(Option.none())
  })

  test("the dropped row does not take the rest of the listing with it", () => {
    const listed = involvedIn("needs-action", [
