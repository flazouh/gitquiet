...
describe("reading how the listed pull requests stand", () => {
  ...
  test("spells the ids the way their route reads them", async () => {
    ...
    expect(calls[0]?.url).toBe(
      "https://github.com/pulls/inbox/deferred?page=1&ids%5B%5D=11&ids%5B%5D=22"
    )
  })
  ...
  test("batches nine at a time, as their own dashboard does", async () => {
    ...
    expect(calls[0]?.url).toContain("ids%5B%5D=9")
    expect(calls[1]?.url).toBe("https://github.com/pulls/inbox/deferred?page=1&ids%5B%5D=10")
  })
  ...
  test("joins every batch into one answer", async () => {
    intercept((url) =>
      Response.json(
        deferredPayload([
          {
            id: url.includes("ids%5B%5D=10") ? "10" : "1",
