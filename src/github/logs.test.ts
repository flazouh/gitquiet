...
import type { LogLine } from "../domain/PullRequest"
import { around, foldedInto, linesIn, pathIn, tailOf, troubleIn } from "./logs"

/** Real lines, timestamps and all, from ci / architecture on ori#1392. */
const real = [
...
describe("reading only the end of a log", () => {
  test("keeps the last lines and says which line they start at", async () => {
    const tail = await tailOf(streamOf(numbered(1000)), 100)
  ...
  test("keeps a short log whole, starting where it starts", async () => {
    const tail = await tailOf(streamOf(numbered(3)), 100)
  ...
  test("does not lose a line that arrived split across two pieces", async () => {
    const tail = await tailOf(streamOf(numbered(50), 17), 100)
...
describe("files a line names", () => {
  ...
  test("lose the runner's checkout path, which is nobody's file", () => {
    const [line] = linesIn(
      "2026-01-01T00:00:00.0Z  at x (/home/runner/work/ori/ori/src/deep/thing.ts:8:2)"
    )
    const named = line!.pieces.flatMap((piece) => (Option.isSome(piece.file) ? [piece.file.value] : []))
