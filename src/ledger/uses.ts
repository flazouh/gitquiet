...
const sureness = (
  ...
  for (const borrowed of told.borrows) {
    if (borrowed.name !== asked.name && borrowed.name !== "*" && borrowed.name !== "default") {
      continue
    }
    if (reaching(path, borrowed.specifier, paths) === asked.path) return "sure"
  }
  ...
  if (told.declares.includes(asked.name)) return "no"
