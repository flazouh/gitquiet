...
const buttonVariants = cva(
  [
    ...
    "transition-[color,background-color,transform] duration-[var(--duration-micro)] ease-[var(--ease-smooth-out)]",
    // The whole box gives under a press, label and icon with it, on the same
    // 0.97 / 160ms as every button in the card. Scaling only the paint layer
    // read as the background flinching away from its own text.
    "active:scale-[var(--scale-press)] active:duration-[var(--duration-press)]",
    ...
    const internals = (
      <>
        <span
          aria-hidden
          className={cn(
            "absolute inset-0 rounded-[inherit] transition-[background-color] duration-[var(--duration-micro)] ease-[var(--ease-smooth-out)]",
            bgClass
          )}
        />
        <span className="relative inline-flex items-center justify-center gap-[inherit]">
          {loading ? (
            ...
            <span className="[&_svg]:stroke-[1.5] [&_svg]:transition-[stroke-width] [&_svg]:duration-[var(--duration-micro)] [&_svg]:ease-[var(--ease-smooth-out)] group-hover:[&_svg]:stroke-[2]">
              {label}
            </span>
          ) : (
            <>
              {LeadingIcon && (
                <LeadingIcon
                  size={iconSize}
                  strokeWidth={1.5}
                  className="transition-[stroke-width] duration-[var(--duration-micro)] ease-[var(--ease-smooth-out)] group-hover:stroke-[2]"
                />
              )}
