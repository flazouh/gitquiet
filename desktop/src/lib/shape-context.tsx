...
const shapeMap: Record<ShapeVariant, ShapeClasses> = {
  ...
  // The window's own ladder, which the app declares at its root: eight pixels on
  // anything that is pressed or typed into, and a wider corner on the thing that
  // holds them. `rounded-lg` is twelve here, so the classes name the radii the
  // interface beside these controls already draws — rows, menus and summaries at
  // eight, cards at ten — rather than the registry's defaults.
  rounded: {
    item: "rounded-md",
    bg: "rounded-md",
