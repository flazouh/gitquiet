...
const visit = async (url: string): Promise<Visit> => {
  const profile = `/tmp/githubpro-bench-${Date.now()}-${Math.random().toString(36).slice(2)}`
...
/** The four routes the gateway reads to open a pull request, as the gateway asks for them. */
const ROUTES = [
  "/changes",
