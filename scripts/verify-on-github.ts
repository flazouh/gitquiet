...
type Measured = {
  ...
  /**
   * Every glyph our part of the page drew, whatever set it came from.
   *
   * The count above cannot be read without this one. Signed out, the screen is two
   * sentences and two buttons and draws no glyph at all, so "no Octicons" was true
   * of a screen with nothing to judge — a gate that passed and failed for reasons
   * that had nothing to do with the set.
   */
...
const look = () =>
  ...
  (() => {
    ...
    const ourBox = document.querySelector("#gitquiet-root .Box")
    ...
    return {
      ...
      glyphs: document.querySelectorAll("#gitquiet-root svg").length,
