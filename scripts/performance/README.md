...
The code-quality review found a link moved into native content before observer delivery. A failing regression test reproduced it. The fix checks final root containment. The final review found no remaining blockers in the observer changes. Live PR rendering, animation and file-tree checks remain pending installation of the changed extension.
